...
class LoginSystem:
    def __init__(self, root):
        ...
        user_name_label = Label(login_frame, text= "Username", compound=LEFT, font=("times new roman", 20, "bold"), bg='white').grid(row=1, column=0, padx=20, pady=10)
        ent_username =  Entry(login_frame, textvariable=self.username, bd=5, relief=GROOVE).grid(row=1, column=1, padx=10)

        password_label = Label(login_frame, text= "Password", compound=LEFT, font=("times new roman", 20, "bold"), bg='white').grid(row=2, column=0, padx=20, pady=10)
